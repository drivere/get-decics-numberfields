...
static GEN
mtsingle_queue_get(struct mt_state *mt, pari_long *workid, pari_long *pending)
{
  GEN done = mt->pending;
...
static void
mtsingle_queue_submit(struct mt_state *mt, pari_long workid, GEN work)
{
...
void
mt_queue_submit(struct pari_mt *pt, pari_long workid, GEN work)
{ pt->submit(&pt->mt, workid, work); }

GEN
mt_queue_get(struct pari_mt *pt, pari_long *workid, pari_long *pending)
{ return pt->get(&pt->mt, workid, pending); }


void
mtstate_save(pari_long *pending)
{
  *pending = mt_is_parallel();
}

void
mtstate_restore(pari_long *pending)
{
