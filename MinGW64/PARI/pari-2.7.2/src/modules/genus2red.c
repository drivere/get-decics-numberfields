...
static const pari_long VERYBIG = (1LL<<20);
static pari_long
...
/* min(a,b,c) */
static pari_long
min3(pari_long a, pari_long b, pari_long c)
{
  pari_long m = a;
  if (b < m) m = b;
  if (c < m) m = c;
...
/* a/b */
static GEN
frac2s(pari_long a, pari_long b) { return b == 1? stoi(a): gdivgs(stoi(a), b); }

/* Vector of p-adic factors (over Q_p) to accuracy r of pol. */
static GEN
padicfactors(GEN pol, GEN p, pari_long r) { return gel(factorpadic(pol,p,r),1); }

/* x(1/t)*t^6, deg x <= 6 */
static GEN
RgX_recip6(GEN x)
{
  pari_long lx = lg(x), i, j;
  GEN y = cgetg(9, t_POL);
  y[1] = x[1];
...
static GEN
factmz(GEN Q, GEN p, pari_long *maxord)
{
  GEN D, z = Q;
  pari_long m = 0;
  if (cmpiu(p,5) <= 0)
...
static pari_long
discpart(GEN H, GEN p, pari_long prec)
{
  GEN list, prod, dis;
  pari_long i, j;
 ...
 * N.B. 60 theta \in Z */
static GEN
theta_j(GEN B, GEN p, pari_long j)
{
...
theta_3_zi(GEN B)
{
  pari_long v2 = myval_zi(truecoeff(B,2));
  pari_long v1 = myval_zi(truecoeff(B,1));
  pari_long v0 = myval_zi(truecoeff(B,0));
...
theta_3_zi2(GEN B)
{
  pari_long v2 = myval_zi2(truecoeff(B,2));
  pari_long v1 = myval_zi2(truecoeff(B,1));
  pari_long v0 = myval_zi2(truecoeff(B,0));
  return frac2s(min3(6*v2, 3*v1, 2*v0), 6);
}
...
polymini(GEN pol, GEN p)
{
  ...
  pari_long alpha, lambda, maxord, beta;
  ...
  for(;;)
  {
    ...
    {
      pari_long ent = itos(gfloor(theta));
...
{
  pari_long alpha, beta, v = MAXVARN;
  GEN a0, a1, a2, a3, a4, a5, a6;
...
struct igusa_p {
  pari_long eps, eps2, tt, r1, r2, R, tame;
...
{
  static const pari_long deg[9] = { 0,2,4,4,6,8,10,12 };
  GEN j2 = I->j2, i4 = I->i4, j6 = I->j6, j8 = I->j8, j10 = I->j10;
  GEN i12 = I->i12, p = Ip->p, val = Ip->val;
  GEN J, v, s, Ieps;
  pari_long r1, r2, r3, r4, i, eps, eps2;
...
static GEN
dicyclic(pari_long a, pari_long b)
{
  pari_long d;
  if (!a) a = 1;
  if (!b) b = 1;
...
static GEN
groupH(pari_long n) { return odd(n)? cyclic(4): dicyclic(2,2); }

static pari_long
get_red(struct red *S, struct igusa_p *Ip, GEN polh, GEN p, pari_long alpha, pari_long r)
{
  GEN val = Ip->val;
  pari_long indice;
  switch(r)
...
static pari_long labelm3(GEN polh, GEN theta, pari_long alpha, pari_long dismin, struct igusa *I, struct igusa_p *Ip);
...
static void
tame_234_init(struct igusa *I, struct igusa_p *Ip, pari_long v12,
                pari_long *pn, pari_long *pq, pari_long *pr, pari_long *flc)
{
  pari_long va0, va5, vb2;
...
/* Ip->tt = 2 */
static pari_long
tame_2(struct igusa *I, struct igusa_p *Ip, pari_long v12)
{
  pari_long condp = -1, d, n, q, r, flc;
  GEN val = Ip->val;
...
/* Ip->tt = 3 */
static pari_long
tame_3(struct igusa *I, struct igusa_p *Ip, pari_long v12)
{
  pari_long condp = -1, n, q, r, flc;
  pari_long va5, d1, d2;
...
/* Ip->tt = 4 */
static pari_long
tame_4(struct igusa *I, struct igusa_p *Ip, pari_long v12)
{
  pari_long condp = -1, d1, d2, d3, f1, f2, g, h, n, q, r, flc;
...
tame_567_init_3(struct igusa_p *Ip, GEN dk,
                pari_long *pd, pari_long *pn, pari_long *pdm, pari_long *pr)
{
  pari_long n = 1 + Ip->r1/6;
  *pd = itos(gmulgs(dk,n));
  *pn = n;
  *pr = -1; /* unused */
  *pdm = 0;
...
tame_6(struct igusa *I, struct igusa_p *Ip, GEN dk,
         GEN polh, GEN theta, pari_long alpha, pari_long dismin)
{
  pari_long condp = -1, d, d1, n, dm, r;
  GEN val = Ip->val, d1k;
...
tame_7(struct igusa *I, struct igusa_p *Ip, GEN dk,
         GEN polh, GEN theta, pari_long alpha, pari_long dismin)
{
  pari_long condp = -1, d, d1, d2, n, dm, r;
...
static pari_long
tame(GEN polh, GEN theta, pari_long alpha, pari_long dismin, struct igusa *I, struct igusa_p *Ip)
{
  GEN val = Ip->val, dk;
  Ip->tame = 1;
  switch(Ip->tt)
...
/* p = 3 */
static pari_long
quartic(GEN polh, pari_long alpha, pari_long dismin, struct igusa_p *Ip)
{
  GEN theta, val = Ip->val, p = Ip->p;
  GEN polf = polymini_zi2(ZX_Z_mul(polh, powiu(p, alpha)));
  pari_long condp = -1, d, R, r1, beta;
  theta = gel(polf,1);
...
static pari_long
litredtp(pari_long alpha, pari_long alpha1, GEN theta, GEN theta1, GEN polh, GEN polh1,
         pari_long dismin, struct igusa *I, struct igusa_p *Ip)
{
  GEN val = Ip->val, p = Ip->p;
  pari_long condp = -1, indice, d, R = Ip->R;
  ...
  {
    struct red S1, S;
    pari_long comp = get_red(&S1, Ip, polh1, p, alpha1, Ip->r1)
...
static pari_long
labelm3(GEN polh, GEN theta, pari_long alpha, pari_long dismin,
        struct igusa *I, struct igusa_p *Ip)
{
  GEN polh1, theta1, polf, val = Ip->val, p = Ip->p;
  pari_long alpha1, lambda, beta, R;
...
/* p = 3 */
static pari_long
quadratic(GEN polh, pari_long alpha, pari_long dismin,
          struct igusa *I, struct igusa_p *Ip)
{
  pari_long alpha1, beta, R;
...
genus2localred(struct igusa *I, struct igusa_p *Ip, GEN p, GEN polmini)
{
  ...
  pari_long i, vb5, vb6, d, dismin, alpha, lambda;
  pari_long condp = -1, indice, vc6, mm, nb, dism;
...
genus2red(GEN Q, GEN P, GEN p)
{
  ...
  for (i = 1; i < l; i++)
  {
    ...
    struct igusa_p Ip;
    pari_long f = genus2localred(&I, &Ip, q, gel(vecmini,i));
