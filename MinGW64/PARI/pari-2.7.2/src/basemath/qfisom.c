...
{
  pari_long s = signe(z);
  return s==0 ? 0: (pari_long)(s*mod2BIL(z));
}
...
static pari_long
possible(GEN F, GEN Ftr, GEN V, GEN W, GEN per, pari_long I, pari_long J)
{
  pari_long i, j, k, count;
  pari_long n = lg(W)-1, f = lg(F)-1;
  count = 0;
  for (j = 1; j <= n; ++j)
  {
...
fingerprint(struct fingerprint *fp, struct qfauto *qf)
{
  ...
  GEN Mf;
  pari_long i, j, k, min;
  pari_long dim = qf->dim, n = lg(V)-1, f = lg(F)-1;
  GEN Ftr;
...
static GEN
bacher(pari_long I, pari_long S, struct qfauto *qf)
{
  pari_sp av=avma;
  GEN V=qf->V, W=qf->W, Fv=gel(qf->v,1);
  GEN  list, listxy, counts, vI;
  pari_long i, j, k, nlist, nxy;
  pari_long n = lg(V)-1;
  pari_long sum, mind, maxd;
  GEN coef;
  ...
  for (i = 1; i <= n; ++i)
    ...
    {
      pari_long s = zv_dotproduct(vI, gel(Fv,i));
      if (s == S)
        list[++nlist] = i;
      if (-s == S)
        list[++nlist] = -i;
  ...
  for (i = 1; i <= nlist; ++i)
  {
    ...
    for (j = 1; j <= nlist; ++j)
    {
      pari_long S2 = list[j] > 0 ? S1 : -S1;
      /* note: for i > 0 is v[-i] = -v[i] */
#ifdef MINGW64
      if (zv_dotproduct(gel(V,llabs(list[i])), gel(Fv,llabs(list[j]))) == S2)  listxy[++nxy] = list[j];
#else
      if (zv_dotproduct(gel(V,labs(list[i])), gel(Fv,labs(list[j]))) == S2)  listxy[++nxy] = list[j];
#endif
    }
    ...
    for (j = 1; j <= nxy; ++j)
    {
      pari_long S1 = listxy[j] > 0 ? S : -S;
      for (k = j+1; k <= nxy; ++k)
      {
        pari_long S2 = listxy[k] > 0 ? S1 : -S1;
#ifdef MINGW64
        pari_long lj = llabs(listxy[j]), lk = llabs(listxy[k]);
#else
        pari_long lj = labs(listxy[j]), lk = labs(listxy[k]);
#endif
...
static GEN
init_bacher(pari_long bachdep, struct fingerprint *fp, struct qfauto *qf)
{
  GEN z = cgetg(bachdep+1,t_VEC);
  pari_long i;
  for (i=1;i<=bachdep;i++)
  {
    pari_long bachscp = mael(qf->W,fp->e[i],1) / 2;
...
static pari_long
bachcomp(GEN pol, pari_long I, pari_long S, GEN V, GEN W, GEN Fv)
{
  pari_sp av = avma;
  GEN co, list, listxy, vI;
  pari_long i, j, k;
  pari_long nlist, nxy, count;
  pari_long n = lg(V)-1;
  pari_long sum = mael(pol,1,1), mind = mael(pol,1,2), maxd = mael(pol,1,3);
  GEN coef = gel(pol,2);
#ifdef MINGW64
  I = llabs(I);
#else
...
static pari_long
operate(pari_long nr, GEN A, GEN V)
{
  pari_sp av = avma;
  pari_long im,eps;
#ifdef MINGW64
  GEN w = zm_zc_mul(A,gel(V,llabs(nr)));
#else
...
static GEN
orbit(GEN pt, pari_long ipt, pari_long npt, GEN H, GEN V)
{
  pari_long i, cnd, im;
  pari_long n = lg(V)-1, nH = lg(H)-1, norb = npt+16, no = npt;
...
static pari_long
orbitlen(pari_long pt, pari_long orblen, GEN G, pari_long nG, GEN V)
{
  pari_sp av = avma;
  pari_long i, len, cnd;
  pari_long n = lg(V)-1;
  ...
  for(cnd = 1; cnd <= len  &&  len < orblen; ++cnd)
    ...
    {
      pari_long im = operate(orb[cnd], gel(G,i), V);
      if (flag[im] == 0)
        /* the image is a new point in the orbit */
      {
        orb[++len] = im;
...
{
  pari_long i, j, len;
  pari_long l1 = lg(orb1)-1;
  pari_long l2 = lg(orb2)-1;
  for (i = 1; i <= l1  &&  orb1[i] != 0; ++i);
  len = i - 1;
  for (i = 1; i <= l2  &&  orb2[i] != 0; ++i)
  {
    pari_long o2i = orb2[i];
...
static pari_long
orbsubtract(GEN Cs, GEN pt, pari_long ipt, pari_long npt, GEN H, GEN V, pari_long *len)
{
  pari_sp av = avma;
  pari_long nC;
...
static void
stab(pari_long I, struct group *G, struct fingerprint *fp, GEN V, ulong p)
{
  pari_long len, cnd, tmplen;
  GEN orb, w, flag, H, Hj, S;
  pari_long i, j, k, l, im, nH, nHj, fail;
  pari_long Maxfail, Rest;
  pari_long dim = lg(fp->diag)-1, n = lg(V)-1;
    ...
    for (i = 1; i <= nH  &&  fail < Maxfail+Rest; ++i)
    {
      if (fail >= Maxfail)
        /* there have already been Maxfail successive failures, now a random
           ...
           stabilizer elements */
      {
        cnd = 1+(pari_long)(pari_rand() % len);
        i = 1+(pari_long)(pari_rand() % nH);
      }
      im = operate(orb[cnd], gel(H,i), V);
      if (flag[im+n+1] == 0)
...
static pari_long
qfisom_candidates_novec(GEN CI, pari_long I, GEN x, struct qfauto *qf,
       struct qfauto *qff, struct fingerprint *fp)
{
  pari_sp av = avma;
  pari_long i, j, k, okp, okm, nr, fail;
  GEN vec;
...
static pari_long
qfisom_candidates(GEN CI, pari_long I, GEN x, struct qfauto *qf,
      struct qfauto *qff, struct fingerprint *fp, struct qfcand *qfcand)
{
  ...
  pari_long vj, rank, num;
  GEN com, list, trans, ccoef, cF;
  GEN F =qf->F, V=qff->V, W=qff->W, v=qff->v, FF= qff->F;
  pari_long dim = qf->dim, n = lg(V)-1, f = lg(F)-1;
  pari_long nc;
  pari_long DEP = qfcand->cdep, len = f * DEP;
  if (I >= 2  &&  I <= lg(qfcand->bacher_pol))
  {
#ifdef MINGW64
    pari_long BACHSCP = mael(W,llabs(x[I-1]),1) / 2;
#else
    pari_long BACHSCP = mael(W,labs(x[I-1]),1) / 2;
#endif
...
static pari_long
aut(pari_long step, GEN x, GEN C, struct group *G, struct qfauto *qf,
                             struct fingerprint *fp, struct qfcand *cand)
{
  pari_long dim = qf->dim;
...
autom(struct group *G, struct qfauto *qf, struct fingerprint *fp,
                                          struct qfcand *cand)
{
  pari_long i, j, step, im, nC, nH, found, tries;
  GEN  x, bad, H;
  pari_long nbad;
  GEN V = qf->V;
  pari_long dim = qf->dim, n = lg(V)-1;
  pari_long STAB = 1;
...
zm_maxdiag(GEN A)
{
  pari_long dim = lg(A)-1;
  pari_long max = coeff(A,1,1);
  pari_long i;
  for (i = 2; i <= dim; ++i)
    if (coeff(A,i,i) > max)
      max = coeff(A,i,i);
...
static GEN
init_qfauto(GEN F, pari_long max, struct qfauto *qf, GEN norm)
{
  pari_long i, j, k;
  GEN W, v;
  GEN M = minim(zm_to_ZM(gel(F,1)), stoi(max), NULL);
  GEN V = ZM_to_zm(gel(M, 3));
  pari_long n = lg(V)-1, f = lg(F)-1, dim = lg(gel(F,1))-1;
  for (i = 1; i <= n; ++i)
  {
    GEN Vi = gel(V,i);
    ...
    {
#ifdef MINGW64
      pari_long l = llabs(Vi[k]);
#else
      pari_long l = labs(Vi[k]);
#endif
      if (l > max)
        max = l;
    }
...
#ifdef MINGW64
        if (llabs(vij[k]) > max) pari_err_OVERFLOW("qfisom [lattice too large]");
#else
 ...
 * them on scpvec  */
static GEN
scpvector(GEN w, GEN b, pari_long I, pari_long dep, GEN v)
{
  pari_long  i, j, n = lg(v)-1;
...
static GEN
scpvecs(GEN *pt_vec, pari_long I, GEN b, pari_long dep, struct qfauto *qf)
{
  pari_long  i, j, nr, sign;
  GEN list, vec;
  GEN vecnr;
  GEN V = qf->V, F = qf->F, v = qf->v;
  pari_long n = lg(V)-1;
  pari_long dim = lg(gel(F,1))-1;
  pari_long len = (lg(F)-1)*dep;
...
scpforms(GEN b, struct qfauto *qf)
{
  pari_long i, j, k;
  GEN F = qf->F;
  pari_long n = lg(F)-1, dim = lg(gel(F,1))-1;
  pari_long nb = lg(b)-1;
...
static GEN
gen_comb(pari_long cdep, GEN A, GEN e, struct qfauto *qf, pari_long lim)
{
  pari_long i, dim = lg(A)-1;
...
{
  GEN V;
  pari_long i, j, n=1, dim = lg(G->ord)-1;
  GEN o = gen_1;
  for (i = 1; i <= dim; ++i)
    o = muliu(o, G->ord[i]);
  for (i = 1; i <= dim; ++i)
...
init_qfisom(GEN F, struct fingerprint *fp, struct qfcand *cand,
                   struct qfauto *qf, GEN flags, pari_long *max)
{
  GEN A, norm;
  if (is_qfisom(F))
  {
...
static GEN
isostab(pari_long pt, GEN G, GEN V, pari_long Maxfail, ulong p)
{
  pari_sp av = avma;
  pari_long  len, cnd, orblen, tmplen, rpt;
  GEN  w, flag, orb;
  pari_long  i, im, nH, fail;
  pari_long dim = lg(gel(V,1))-1, n = lg(V)-1, nG = lg(G)-1;
  GEN H;
...
static pari_long
iso(pari_long step, GEN x, GEN C, struct qfauto *qf, struct qfauto *qff,
...
{
  pari_sp ltop=avma;
  pari_long rows = nbrows(M), cols = lg(M)-1;
  pari_long i, j, c;
...
GEN
qfautoexport(GEN G, pari_long flag)
{
  pari_sp av = avma;
  pari_long i, lgen,  c = 2;
  ...
  {
    pari_long dim = lg(gmael(gen,1,1))-1;
