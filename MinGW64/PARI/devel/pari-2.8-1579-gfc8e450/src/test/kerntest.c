...
{
#ifdef LONG_IS_64BIT
  if (x>>32) printf("%08I64x  ", x>>32);
  printf("%08I64x  ", x & 0xFFFFFFFF);
#else
  printf("%08I64x  ", x);
#endif
}
...
{
  long i, lx = lg(x);
  printf("signe: %I64d, expo: %I64d, ",signe(x),expo(x));
...
int main()
{
  GEN x,y,r,z, xr,yr;
  specinit();
  x = utoipos(187654321ULL);
  y = utoineg(12345678ULL);
  printf("INT: %I64d\n", itos(x));
