...
void Mart52Engine_Tgt(GEN K, GEN CVecVec, pari_long dK, pari_long dL_Factor, int ChkPntFlag, pari_long CvecIdx, 
  ...
  {
  int  a1_is_0,j,Idx;
  pari_long ltop,ltop1,ltop2,ltop3,ltop4,ltop5;
  ...
  GEN Cvec_prime,kvec,Lvec,mvec,Nvec;
...
************************************************************************************/
  ...
  w_alg = basistoalg(K,mkcol2(gen_0,gen_1));  // Convert w into algebraic number (represented as a pari t_POLMOD)
  w_sq  = algtobasis(K,gsqr(w_alg));          // Square w and convert back to integral basis format.
  wsq_1 = itos((GEN)w_sq[1]);  // w^2 = wsq_1 + wsq_2 * w where
  wsq_2 = itos((GEN)w_sq[2]);
  // This is w and its Galois conjugate as complex numbers:
  w  = gmael(Q,2,1);  // row 1, col 2
  wc = gmael(Q,2,2);  // row 2, col 2  (the Galois conjugate of w)
  ...
  P = idealhermite(K,gmael(Mat,1,1)); // 1st prime from Mat (in Hermite normal form)
  ...
  Minv = ginv(M);  // The inverse of the matrix M
  ...
  B = gmul(gmul(gtrans(M),QHQ),M);  // This is (Q*M)^H * (Q*M)
  ...
  // Allocate memory for the polynomial buffer and the polGoodFlag
  pari_long *polyBuffer = (pari_long*)malloc(polyBufferSize * 11 * sizeof(pari_long));
  if (!polyBuffer) {
    fprintf(stderr, "Failed to allocate polynomial buffer.\n");
    exit(1);
  }
                                                ...
                                                if(bb[9]>=bb9_L && bb[9]<=bb9_U)
                                                  ...
                                                  if(bb[8]>=bb8_L && bb[8]<=bb8_U)
                                                    ...
                                                    if(buffCnt==polyBufferSize)
                                                      {
                                                      // Test every polynomial in the buffer.
                                                      pari_long loopIdxs[9] = {i,N1,N2,k1,k2,L1,L2,m1+1,m2};
  ...
  if(buffCnt>0) {
    ...
    pari_long loopIdxs[9] = {i,N1,N2,k1,k2,L1,L2,m1,m2};
    testPolys(polyBuffer, buffCnt, StatVec, loopIdxs, outfile);
    }
...
void testPolys(pari_long *polBuf, int numPolys, pari_long *StatVec, pari_long *loopIdxs, ofstream& outfile)
  {
  ...
  *polCount += numPolys;  // Update the polynomial counter.
  ...
  int status = polDiscTest_cpu(polBuf, numPolys, polGoodFlag, numPrimes, pSet);
#endif

  // This performs the polDisc test using the CUDA-GPU version:
#ifdef APP_VERSION_GPU_CUDA
  boinc_begin_critical_section();
  int status = polDiscTest_gpuCuda(polBuf, numPolys, polGoodFlag, numPrimes, pSet);
  ...
  boinc_begin_critical_section();
  int status = polDiscTest_gpuOpenCL(polBuf, numPolys, polGoodFlag, numPrimes, pSet);
  boinc_end_critical_section();
#endif
  ...
  for(long j=0;j<numPolys;++j)
    ...
    if(polGoodFlag[j])
      ...
      GEN f_L = mkpoln( 11, gen_1, stoi(polBuf[j*11+1]), stoi(polBuf[j*11+2]),
                                   stoi(polBuf[j*11+3]), stoi(polBuf[j*11+4]),
                                   stoi(polBuf[j*11+5]), stoi(polBuf[j*11+6]),
                                   stoi(polBuf[j*11+7]), stoi(polBuf[j*11+8]),
                                   stoi(polBuf[j*11+9]), stoi(polBuf[j*11+10]) );
  ...
  if(boinc_time_to_checkpoint())
    ...
    StatVec[4] = StatVec[4] + (pari_long)floor(timer()/1000.0 + .5);

    int retval = do_checkpoint(loopIdxs, StatVec);
    if(retval)
      {
