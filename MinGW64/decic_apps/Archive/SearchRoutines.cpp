...
int MartinetSearch(char *FilenameIn, char *FilenameOut, int ChkPntFlag, int a5_Start_Idx,
  ...
  /*
  // Remove any temporary files created by mpqs during a previous run
  string tmpdir = string(pari_unique_dir("MPQS"));
  //cout << "tmpdir = " << tmpdir << "\n";
#ifdef _WIN32
  long I1 = tmpdir.rfind("MP");  // Get index of start of MPQS string
  string Command = "RD /S /Q " + tmpdir.substr(0,I1) + "MP*";
#else
  long I1 = tmpdir.rfind("MPQS");  // Get index of start of MPQS string
  string Command = "rm -rf " + tmpdir.substr(0,I1) + "MPQS*";
#endif
  //cout << "Command = " << Command << "\n";
  int stat = system(Command.c_str());
  if(stat==-1) { cout << "  MPQS cleanup failed! Aborting.\n"; exit(1); }
  */
    ...
    if(NumVals_a22==1)  // Then load the limits for a21
      ...
      if(NumVals_a21==1)  // Then load the limits for a32
        ...
        if(infile.eof())  // Then this is an old style WU (a32_L,a32_U are not present)
          ...
          {
          infile >> a32_U;
          fprintf(stderr, "    a32_L = %lld\n",a32_L);
          fprintf(stderr, "    a32_U = %lld\n",a32_U);
...
void Mart52Engine_Std(GEN K, pari_long dK, int a11, int a12, pari_long a51, pari_long a52, pari_long a22_L0, pari_long a22_U0, 
  ...
  for(a22=a22_L;a22<=a22_U;++a22)
    ...
    for(a21=a21_L;a21<=a21_U;++a21)
      ...
      if(NumVals_a5==1 && NumVals_a22==1)  fprintf(stderr, "    a21 = %lld.\n",a21);
      ...
      if( (s1a2_mag+s2a2_mag) <= 2.0*Ca1 )
        ...
        if(bb2>=bb2_L && bb2<=bb2_U)
          ...
          for(a32=a32_L;a32<=a32_U;++a32)
            ...
            //fprintf(stderr, "            a31_L = %lld.\n",a31_L);
            //fprintf(stderr, "            a31_U = %lld.\n",a31_U);

            ltop2 = avma;

            for(a31=a31_L;a31<=a31_U;++a31)
              {
              //fprintf(stderr, "              a31 = %lld.\n",a31);
              a31sq  = a31*a31;
              a32sq  = a32*a32;
              a31a32 = a31*a32;
              ...
              if( (s1a3_mag<=2*tq[3]) && (s2a3_mag<=2*tqc[3]) )
                ...
                if(bb3>=bb3_L && bb3<=bb3_U)
                  ...
                  //fprintf(stderr, "                a42_L = %lld.\n",a42_L);
                  //fprintf(stderr, "                a42_U = %lld.\n",a42_U);
                                        ...
                                        if(!gcmp0(PolDisc))  // Then PolDisc is nonzero
                                          ...
                                          if(issquare(gdiv(PolDisc,dLg)))
                                            ...
                                            if(isirreducible(f_L))
                                              ...
                                              if(gequal(dLg,gabs(nfdisc(f_L),BIGDEFAULTPREC)))
                                                {
                                                ++Test3_Count;
                                                OutStr = GENtostr(f_L);
                                                outfile << OutStr << "\n";
