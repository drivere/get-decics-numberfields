...
// This version uses a 128bit digit and 256bit word.
...
// It is the number of mp_digits. A "digit" is 128bits.
// But only 127 bits of each digit is actually used.  So 127*6 = 889 bits max.
#define MP_PREC  7
...
typedef uint128_t  mp_digit;
typedef uint256_t  mp_word;
#define DIGIT_BIT  (uint)127
#define MP_MASK  ((((mp_digit)1)<<(DIGIT_BIT))-((mp_digit)1))
...
__device__ int  mp_div_2d(mp_int*, uint, mp_int*, mp_int*);
__device__ int  mp_mod_2d(mp_int*, uint, mp_int*);
...
int fast_s_mp_mul_digs(mp_int *a, mp_int *b, mp_int *c, int digs)
{
   ...
   //printf("      Entering fast_s_mp_mul_digs\n");
...
__device__ inline
int mp_mul_2d(mp_int *a, uint b, mp_int *c)
{
   //printf("      Entering mp_mul_2d\n");
...
int mp_div(mp_int *a, mp_int *b, mp_int *c, mp_int *d)
{
   ...
   uint norm = mp_count_bits(&y) % DIGIT_BIT;
   if (norm < (DIGIT_BIT - 1)) {
...
#pragma unroll
   for (i = n; i >= (t + 1); i--) {
      ...
      if (x.dp[i] == y.dp[t]) {
         q.dp[(i - t) - 1] = ((mp_digit)1 << DIGIT_BIT) - (mp_digit)1;
      } else {
         mp_word tmp;
         tmp = (mp_word)x.dp[i] << DIGIT_BIT;
         tmp |= (mp_word)x.dp[i - 1];
         tmp = tmp / y.dp[t];
         if (tmp > (mp_word)MP_MASK)  tmp = (mp_word)MP_MASK;
      ...
      */
      ...
      do {
         ...
         /* find left hand */
         mp_zero(&t1);
         t1.dp[0] = ((t - 1) < 0) ? (mp_digit)0 : y.dp[t - 1];
         t1.dp[1] = y.dp[t];
         t1.used = 2;
         if( mp_mul_d(&t1, q.dp[(i - t) - 1], &t1) != MP_OKAY )  return MP_MEM;

         /* find right hand */
         t2.dp[0] = ((i - 2) < 0) ? (mp_digit)0 : x.dp[i - 2];
         t2.dp[1] = ((i - 1) < 0) ? (mp_digit)0 : x.dp[i - 1];
         t2.dp[2] = x.dp[i];
         t2.used = 3;
...
__device__ inline
int mp_div_2d(mp_int *a, uint b, mp_int *c, mp_int *d)
{
   mp_digit r, rr;
   int x;
...
__device__ inline
int mp_mod_2d(mp_int *a, uint b, mp_int *c)
{
   int x;
...
#pragma unroll
   ...
   c->dp[b / DIGIT_BIT] &= ((mp_digit)1 << (b % DIGIT_BIT)) - (mp_digit)1;
